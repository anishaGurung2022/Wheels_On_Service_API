...
global $con;
if(isset($_POST['name']) && isset($_POST['phone']) && isset( $_POST['address']) && isset( $_POST['email']) && isset($_POST['userName']) && isset($_POST['password'])){
    $name = $_POST['name'];
    $phone = $_POST['phone'];
    ...
    if(strpos($email,"@gmail.com") !== false && preg_match('/^[0-9]{10}+$/', $phone)){
        ...
        if ($count > 0) {
            ...
            signup($name,$phone,$address,$email,$userName,$password);
        }
    }
    else{
        echo json_encode(
